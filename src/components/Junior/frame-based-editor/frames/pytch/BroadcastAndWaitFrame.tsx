...
const BroadcastAndWaitFrame = ({ frame, editFrame, setIsEditingText, actorId, handlerId }: BroadcastAndWaitFrameProps) => {
	...
	return (
		<div>
			pytch.broadcast_and_wait("<input spellCheck={false} onFocus={() => setIsEditingText(true)} onBlur={() => setIsEditingText(false)} value={frame.message} onChange={changeValueInput} />")
		</div>
	)
}
